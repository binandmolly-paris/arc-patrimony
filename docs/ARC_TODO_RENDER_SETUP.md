...
ARC TODO 有两层结构：**项目**用来保存一个较大的目标、说明和可选总截止日，例如“更新跨机构资料”；**子任务**才是实际执行项，例如“列出机构清单”或“办理某一项资料更新”。

项目本身不会触发邮件、日历或专注提醒，也不会占用个人计划时间。每个子任务仍可独立分派给刘斌、Molly 或 Yukun，并拥有自己的截止日、提醒、个人排程和当前重点。旧任务不属于任何项目，继续作为独立任务存在。
